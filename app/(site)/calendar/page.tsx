    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <Link href={`/calendar?year=${next.y}&month=${next.m}`} className="px-3 py-2 rounded-block border hover:bg-gray-50">{monthLabel(next.y, next.m)} →</Link>
        </div>
        <form className="flex items-center gap-2" action="/calendar">
          <input type="hidden" name="day" value="" />
          <select name="month" defaultValue={month} className="border rounded-block px-2 py-1">
            {Array.from({length:12}).map((_,i)=>(<option key={i+1} value={i+1}>{new Date(2000,i,1).toLocaleString(undefined,{month:'long'})}</option>))}
          </select>
          <select name="year" defaultValue={year} className="border rounded-block px-2 py-1">
            {Array.from({length:7}).map((_,i)=>{ const y=now.getFullYear()-3+i; return <option key={y} value={y}>{y}</option>})}
          </select>
      ...
      <div className="grid grid-cols-7 gap-2">
        {cells.map((cell, idx) => {
          ...
          return (
            <div key={idx} className={`border rounded-block bg-white p-2 min-h-[72px] ${isSelected ? 'ring-2 ring-mc-grass' : ''}`}>
              {typeof d === 'number' ? (
                <div className="flex items-center justify-between">
                  <div className="text-sm font-semibold">{d}</div>
                  {count > 0 && (
                    <Link href={`/calendar?year=${year}&month=${month}&day=${d}`} className="text-xs bg-mc-grass text-white rounded-full px-2 py-0.5">
              ...
              {typeof d === 'number' && (
                <Link href={`/calendar?year=${year}&month=${month}&day=${d}`} className="block mt-4 text-xs text-mc-stone hover:underline">
                  View day →
                </Link>
              )}
            </div>
      ...
      {selectedDay && (
        <div className="mt-6">
          ...
          <div className="mt-3 space-y-3">
            {postsForDay.length === 0 && <p className="text-sm text-mc-stone">No posts on this day.</p>}
            {postsForDay.map(p=>(
              <div key={p.id} className="card-block p-4">
                ...
                <Link href={`/post/${p.id}`} className="mt-2 inline-block underline text-sm">Open post</Link>
              </div>
            ))}
          </div>
        </div>
