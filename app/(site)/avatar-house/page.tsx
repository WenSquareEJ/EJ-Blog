...
export default async function Page({
  ...
  return (
    <div className="mx-auto max-w-4xl p-6 space-y-6">
      ...
      <form action={saveAvatar} method="post" className="home-card p-4 space-y-5">
        {/* Secret field */}
        ...
        {/* Avatar grid */}
        <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-6 gap-5 justify-items-center">
          {AVATAR_CHOICES.map((stem) => {
            ...
            return (
              <button
                ...
                aria-label={stem}
                className={[
                  // Sizing: ~150–180px squares depending on breakpoint
                  "relative w-36 h-36 sm:w-40 sm:h-40 aspect-square",
                  // Shape / border / bg
                  "rounded-xl border-[3px] p-2 bg-white/95",
                  // Interaction / glow
                  "transition-all duration-200 hover:brightness-105 hover:ring-4 hover:ring-[#2f6f32]/40 hover:shadow-[0_0_12px_#2f6f32aa]",
                  // Default border color
                  "border-[#5a3d1a]",
                  // Selected state
                  selected ? "border-[#2f6f32] ring-4 ring-[#2f6f32]/60 shadow-[0_0_15px_#2f6f32aa] scale-105" : "",
                ].join(" ")}
              >
                <img
                  src={url}
                  ...
                  style={{ imageRendering: "pixelated", padding: 6 }}
                />
              </button>
            );
          })}
